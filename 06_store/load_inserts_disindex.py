...
def createTable(conn):
	...
	with conn.cursor() as cursor:
		...
		print(f"Created {TableName} without primary key and indexes")
...
def apply_indexes_and_primary_key(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
            ALTER TABLE {TableName} ADD PRIMARY KEY (CensusTract);
            CREATE INDEX idx_{TableName}_State ON {TableName}(State);
            """)
        print(f"Primary key and indexes to {TableName} after loading data.")
...
def main():
    ...
    load(conn, cmdlist)
    apply_indexes_and_primary_key(conn)  # Add this line
